...
export default function ProfilePage() {
  ...
  /* ---------------- Delete account flow ----------------
     ...
     2) Clerk delete (step-up via useReverification)
     3) Fallback to /api/account/delete (server-side) if 403
     4) signOut -> redirect "/"
  ------------------------------------------------------ */
  const doDeleteAccount = async () => {
    ...
    try {
      ...
      // 2) Try client-side Clerk delete (this will step-up because we wrap with useReverification)
      try {
        await user.delete()
      } catch (err: any) {
        const msg = String(err?.message || "")
        const status = (err as any)?.status
        const needsStepUp = msg.toLowerCase().includes("additional verification") || status === 403
        if (needsStepUp) {
          // 3) Fallback: ask server to delete with admin key (make sure /api/account/delete exists)
      ...
      // 4) Ensure local session is cleared and redirect home
  ...
  // Wrap destructive action with Clerk step-up (opens OTP/password prompt when needed)
