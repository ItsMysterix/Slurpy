...
const serverEnabled = !!(process.env.SENTRY_DSN || process.env.NEXT_PUBLIC_SENTRY_DSN) && process.env.NEXT_PUBLIC_E2E_BYPASS_AUTH !== "true";
if (serverEnabled) {
  Sentry.init({
    dsn: process.env.SENTRY_DSN || process.env.NEXT_PUBLIC_SENTRY_DSN || undefined,
    tracesSampleRate: process.env.NODE_ENV === "production" ? 0.1 : 1.0,
    // Disable potentially problematic auto instrumentation on Next.js server functions/components
    // to avoid interfering with route handlers and revalidate in dev/test.
...
// Expose minimal global for helpers
// @ts-ignore
(globalThis as any).__SENTRY__ = {
  captureException: (e: unknown) => {
    try { if (serverEnabled) Sentry.captureException(e); } catch {}
  },
};
