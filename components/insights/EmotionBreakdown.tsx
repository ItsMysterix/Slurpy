...
export default function EmotionBreakdown({ breakdown }: Props) {
  const items = breakdown?.emotions ?? [];
  if (!items.length) {
  ...
  return (
    <div className="space-y-3">
      {items.map((emotion) => {
        ...
        const src = iconForEmotion(emotion.emotion); // maps emotion -> /images/fruits/....png

        return (
          <div key={emotion.emotion} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {/* Use <img> for dynamic public assets + fallback */}
              <img
                src={src}
                alt={emotion.emotion}
                className="w-6 h-6 rounded"
                onError={(e) => {
                  (e.currentTarget as HTMLImageElement).src = "/Slurpy.ico";
                }}
              />
              <span className="capitalize text-sm text-clay-700 dark:text-sand-200">
                {emotion.emotion}
              </span>
              <span className="text-sm text-clay-600 dark:text-sand-300">
                {emotion.count} times
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-clay-500 dark:text-sand-400 w-8 text-right">
                {Math.round(widthPct)}%
              </span>
            </div>
          </div>
